...
| 层 | 代码量 | 能力 |
|---|---|---|
| C99 引擎 | 4926 行 | HTML 解析 · CSS 级联 · flex/行内布局 · 绘制指令(5 种) · 动画插值 · 结构自检 |
| macOS 运行时 | ~3900 行 Swift | CoreGraphics 落屏 · 窗口/弹窗/图层物化 · FreeType · JavaScriptCore · sys:// 桥 · hx-* · 流式视图 |
| 软件光栅器 | ~470 行 C | SDF 圆角矩形 · 线性渐变 · FreeType 文本 · PNG 编码 |
| JS 运行时 | ~400 行 Swift | JavaScriptCore · DOM 桥 · 事件派发 |
| 测试 | 1306 行 | 127 项断言 |
| 示例 | 11 个 | 仪表盘 / 网页写法 / 轨迹面板 / 消息卡 / 输入控件 |
...
| **v0.2** | **可用工具** | ~10 周 | 定位 · 省略号 · 逐侧边框 · 事件系统 · 网络 · 表单 · 增量布局 |
| **v0.3** | **应用框架** | ~10 周 | 组件 · 状态 · 路由 · 伪元素 · Grid |
